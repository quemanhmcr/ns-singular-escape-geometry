...
1. Fix a scale-consistent packet/localization framework, then seek a dynamical bound for the forcing-level action \(\mathfrak N_r=\int E\|\Lambda^{-1}\mathcal L F\|_2^2dt\), which directly controls all upward variation of \(K\); at a two-shell profile this is relative off-shell normal forcing.
